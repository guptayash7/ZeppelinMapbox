...
case class mapRenderer4(data: String , rollup : String ,lat: String , lon:String , filter : List[String] , distinctVal : List[String] , weight: String ,  metric :String ="")  {
    ...
    <html>
      ...
      <body>
       ...
       // Transition from heatmap to circle layer by zoom level
          ...
          function myfunc0(v1){val1 = v1;}
          function myfunc1(v2){val2 = v2;}
          function myfunc2(v3){val3 = v3;}
          function myfunc3(v4){val4 = v4;}


          function myfunc() {
              send(val1,val2,val3,val4);
              render();
              var actual_count=0;
              for(var i=0 ; i<actual_data.length ; i++){
                if(actual_data[i].${filter(0)}==val1 && actual_data[i].${filter(1)}==val2 && actual_data[i].${filter(2)} === val3 && actual_data[i].${filter(3)} === val4 ){
              ...
              if("${metric}" != "def"){
                  ...
                  var newvar = actual_data.filter(v => (v.${filter(0)} == val1 && v.${filter(1)} == val2 && v.${filter(2)} == val3 && v.${filter(3)} == val4 ) )
  ...
  function send(val1,val2,val3,val4){
       ...
       zoom = 6;
        ...
        $$.each(data,function (i, f) {
           ...
           if(f.${filter(0)} == val1 && f.${filter(1)} == val2 && f.${filter(2)} == val3 && f.${filter(3)} == val4 ) {
                  lat += f['${lat}'];
                  lon += f['${lon}'];
                  total = total+1;
                  all_points1.push({
  ...
  var map = new mapboxgl.Map({
    ...
    <div style ="align : right;position: relative;align-items: right;padding-left: 85%;padding-top:20px;">
      ...
      <select onchange="myfunc3(this.value)" id = "sel3">
      ...
      <script>
        ...
        var option = document.createElement("OPTION"),
          ...
          option.appendChild(txt);
          option.setAttribute("value",arr[i]);
          sel3.insertBefore(option,sel3.lastChild);
        }
    </script>
