...
case class mapRenderer2(data: String ,rollup :String, lat: String , lon:String , filter : List[String] , distinctVal : List[String] , weight: String , metric: String ="")  {
       ...
       // Transition from heatmap to circle layer by zoom level
          ...
          function myfunc() {
              ...
              for(var i=0 ; i<actual_data.length ; i++){
                if(actual_data[i].${filter(0)}==val1 && actual_data[i].${filter(1)}==val2 && actual_count < actual_data[i].count){
            ...
            if("${metric}" != "def"){
              ...
              var newvar = actual_data.filter(v => (v.${filter(0)} == val1 && v.${filter(1)} == val2))
  ...
  function send(val1,val2){
       ...
       zoom = 6;
        ...
        $$.each(data,function (i, f) {
           ...
           if(f.${filter(0)} == val1 && f.${filter(1)} == val2 ) {
                  lat += f['${lat}'];
                  lon += f['${lon}'];
                  total = total+1;
                  all_points1.push({
...
"""
  ...
  def frameHTML(name: String = "default") = {
    ...
    s"""
       |  <iframe id="${frameName}" height="800px" width = "100%" scrolling = "no" srcdoc="${xml.Utility.escape(pageHTML())}"></iframe>
