...
case class mapRenderer1(data: String ,rollup :String, lat: String , lon:String , filter : String , distinctVal : String , weight: String , metric: String)  {
       ...
       // Transition from heatmap to circle layer by zoom level
          ...
          function myfunc() {
              ...
              for(var i=0 ; i<actual_data.length ; i++){
                if(actual_data[i].${filter}==val1 && actual_count < actual_data[i].count){
            ...
            if("${metric}" != "def"){
              ...
              var newvar = actual_data.filter(v => v.${filter} == val1)
